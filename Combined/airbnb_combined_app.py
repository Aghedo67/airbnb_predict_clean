...
@st.cache_data
def get_freq_encoding(neighbourhood, property_type, _df):
    n_freq = _df[_df['neighbourhood'] == neighbourhood]['neighbourhood_freq'].mean()
    p_freq = _df[_df['property_type'] == property_type]['property_type_freq'].mean()
    if pd.isna(n_freq):
        n_freq = _df['neighbourhood_freq'].mean()
    if pd.isna(p_freq):
        p_freq = _df['property_type_freq'].mean()
